...
func (c *OpenAiClient) SimpleCompletion(model string, roleHint string, input string, messageParts chan<- client.MessagePart) error {
	url := fmt.Sprintf("%s/v1/chat/completions", viper.GetString("openAiUrlPrefix"))
...
func (c *OpenAiClient) Embeddings(model string, input string) (vectors.Vector, error) {
